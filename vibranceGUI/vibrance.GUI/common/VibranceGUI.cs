...
namespace vibrance.GUI.common
{
    public partial class VibranceGUI : Form
    {
        ...
        private const string AppName = "VibranceGammaGUI";
        private const string AppVersion = "2.0.0";
        private const string TwitterLink = "";
        private const string PaypalDonationLink = "https://www.paypal.com/donate/?hosted_button_id=C8MSD3F55XDPY";
            ...
            Func<int, string> resolveLabelLevel)
        {
            ...
            this.notifyIcon.Icon = Resources.vibranceguiplus_logo;
            ...
            // Initialize Authentic NVAPI Probe
            NvidiaColorController.TryInit();

            // Ensure the event hook ignores the application's own process
            WinEventHook.GetInstance().WinEventHookHandler += (s, e) =>
            {
                if (e.ProcessId != Process.GetCurrentProcess().Id)
                {
                    GammaRamp_WinEventHook(s, e);
                }
            };
        ...
        private void Form1_Resize(object sender, EventArgs e)
        {
            if (this.WindowState == FormWindowState.Minimized)
            {
                ...
                this.notifyIcon.BalloonTipText = "Like the program? Right-click and select 'Support Developer' to donate!";
                this.notifyIcon.BalloonTipTitle = "VibranceGammaGUI v2.0.0";
        ...
        private void donateToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                System.Diagnostics.Process.Start(PaypalDonationLink);
            }
            catch (Exception ex)
            {
                MessageBox.Show("Could not open donation link.\n\nPlease visit: " + PaypalDonationLink, "Donation Link", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        ...
        {
            if (_gammaController == null) return;

            // Ignore events from our own process to prevent immediate reversion when focusing the UI
            if (e.ProcessId == (uint)Process.GetCurrentProcess().Id)
            {
                return;
            }
        ...
        private void CleanUp()
        {
            ...
            {
                // Exception handled silently for release
            }
        }
        ...
        private void ReadVibranceSettings(out int vibranceWindowsLevel, out bool affectPrimaryMonitorOnly, out bool neverSwitchResolution)
        {
            ...
            if (this.IsHandleCreated)
            {
                ...
                foreach (ApplicationSetting application in _applicationSettings.ToList())
                {
                    ...
                    using (Icon icon = Icon.ExtractAssociatedIcon(application.FileName))
                    {
                        if (icon != null)
                        {
        ...
        private void buttonAddProgram_Click(object sender, EventArgs e)
        {
            ...
            {
                using (Icon icon = Icon.ExtractAssociatedIcon(fileDialog.FileName))
                {
                    if (icon != null)
                    {
                        ProcessExplorerEntry processExplorerEntry = new ProcessExplorerEntry(fileDialog.FileName, (Icon)icon.Clone(), Path.GetFileNameWithoutExtension(fileDialog.FileName));
        ...
        {
            if (this.listApplications.SelectedItems.Count == 0) return;
